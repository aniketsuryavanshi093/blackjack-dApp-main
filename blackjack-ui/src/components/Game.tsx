...
export const Game: React.FC<IProps> = ({ library, account }) => {
  ...
  const unlockBet = async () => {
    try {
      const signer = new ethers.Wallet(
        "a061485fe53c62c029faefe7bd8b064a1c6d0e6e44c7cc408287048f92c887cf",
        library
      )
      ...
      console.log(signerAddress, blackjackContract);
      ...
      if (score! > 0) {
        ...
        console.log(endGame, endGameReceipt, "won");
        ...
        const endGameReceipt = await library.waitForTransaction(endGame.hash)
        console.log(endGame, endGameReceipt, "tie");
        ...
        const endGameReceipt = await library.waitForTransaction(endGame.hash)
        console.log(endGame, endGameReceipt, "lost");
  ...
  const getWinner = () => {
    ...
    console.log(isGameEnded, tempDeck);
  ...
  const startGame = async () => {
    try {
      ...
      console.log(signer, blackjackContract, tx);
      ...
      >
        <div className=" lg:col-start-1 pt-5 md:mt-0 items-center flex flex-col ">
          ...
          {!isCanWithdraw && roundText[0] !== "Wait for" && account && (
            <button
              className={`${isGameActive ? "hidden" : "md:mt-4"
        ...
        <div className=" md:col-start-2 flex flex-col justify-center items-center">
            ...
            {houseCards?.length !== 0 ? (
              houseCards?.map((card, index) => {
                if (index == 0) {
                  return (
                    <div
                      className={`${isStand ? "transition translate-x-1 duration-300" : ""
                        }`}
                      key={index}
                    >
                      <Image
                  ...
                  return (
                    <div
                      key={index}
                      className={`${index !== 0 ? "-ml-[8rem] md:-ml-[10.5rem]" : ""
                        }  `}
                    >
                      <Image
                        src={card}
          ...
          <div className="flex flex-row w-full -my-2 justify-evenly items-center ">
            {roundText && !loading && (
              <span
                className={` text-3xl w-1/3 text-right ${roundText[0] === "It's a" ? "" : ""
                  }  mt-4 font-sans  `}
              >
                {roundText[0]}
              </span>
            ...
            <div
              className={`${roundText ? "" : " "} ${roundText && roundText[0] === "It's a" ? "" : ""
                } mt-4 md:mt-10 w-1/3 ${loading ? "opacity-60" : "opacity-20"
        ...
        <div className="col-start-2  md:row-start-3 lg:row-start-1 lg:col-start-3   mt-4 md:mt-8 mr-4 md:mr-0 gap-4  flex justify-center  items-center  lg:mr-0 lg:flex-col lg:content-end">
          ...
          {isGameActive && playerSum > 0 && currentDeck.length > 0 && (
            <button
              className={`${playerSum >= 21 ? "cursor-not-allowed " : "cursor-pointer"
                } hover:scale-110 transition duration-200`}
